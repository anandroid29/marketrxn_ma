...
11 real M&A announcements, dates verified against SEC filings, company press releases, and regulator statements — see [`events.csv`](./events.csv). Mix of Indian and global banking/financial-services deals, plus one non-financial control (Broadcom–VMware).
...
- **The median CAR is negative or near-zero at every point in the window** — the "typical" deal in this sample never showed the positive reaction the mean suggests.
- **With First Citizens excluded, CAAR turns significantly negative by day +3 through +5** (p < 0.05). This is consistent with the well-documented "acquirer overpayment" / winner's-curse finding in the M&A literature — acquirers, on average, tend to underperform in the days following a deal announcement.
...
- **The market model uses a single index per deal's home market** (Nifty 50, S&P 500, or SMI) — no adjustment for size, sector, or industry-specific benchmarks (e.g. a bank sub-index).
